...
static public class NGUIText
{
	...
	static public Vector2 CalculatePrintedSize (string text, ref Vector2 lastPos)
	{
		...
		if (!string.IsNullOrEmpty(text))
		{
            ...
            lastPos.x = x - finalSpacingX;
            lastPos.y = y + finalLineHeight;
	...
	/// <summary>
	/// Added by rongzheng 2014/11/27.
	/// </summary>

	static public string ClampStringDotDotDot(string text)
	{
		if (string.IsNullOrEmpty(text) || regionWidth < 1) return "";
		...
		//remove more chars to make room for "..."
		float dotWidth = GetGlyphWidth('.', '.') * 3;
		while (currentCharacterIndex > 0 && remainingWidth < dotWidth)
		{
			remainingWidth += mSizes[--currentCharacterIndex];
		}
		mSizes.Clear();

		return text.Substring(0, currentCharacterIndex) + "...";
	}

	static private void CalculateSizes(string text)
	{
		int textLength = text.Length, ch = 0, prev = 0;

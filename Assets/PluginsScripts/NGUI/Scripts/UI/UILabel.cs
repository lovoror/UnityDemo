...
public class UILabel : UIWidget
{
    ...
    [HideInInspector][SerializeField] string mkey = "";
    ...
    /// <summary>
    /// Record position of last char. Curretly only use for UIImageLabel
    /// </summary>
    [NonSerialized] public Vector2 LastPos = Vector2.zero;
    ...
    private void SetContentKey()
    {
        ...
        {
            string strKey = TextData.getInstance().GetLanguageContentKey(text);
            if (strKey != "")
                Key = strKey;
            else
                Key = "";
    ...
    protected override void OnStart ()
	{
        ...
        if (!bSetText && !string.IsNullOrEmpty(Key) /*&& GameKernel.Get<IDataCenter>() != null && GameKernel.Get<IDataCenter>().GetDataType<ITextData>() != null*/) 
        {
            string strDesc = TextData.getInstance().GetLanguageContent(Key);//GameKernel.Get<IDataCenter>().GetDataType<ITextData>().GetLanguageContent(Key);
            if (strDesc != "")
                text = strDesc;
        }
#if UNITY_EDITOR
	...
	void ProcessText (bool legacyMode, bool full)
	{
		...
		if (mPrintedSize > 0)
		{
			...
			for (int ps = mPrintedSize; ps > 0; --ps)
			{
				...
				{
					mCalculatedSize = NGUIText.CalculatePrintedSize(mProcessedText, ref LastPos);
				...
				else if (mOverflow == Overflow.DotDotDot)
				{
					NGUIText.encoding = false;
					NGUIText.symbolStyle = NGUIText.SymbolStyle.None;
					mProcessedText = NGUIText.ClampStringDotDotDot(text);
					mCalculatedSize = NGUIText.CalculatePrintedSize(mProcessedText, ref LastPos);
				}
				else
				{
	...
	public override void OnFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (m_callback!=null) //added LG 修改支持图文混排
        {
            m_callback();
        }
	}

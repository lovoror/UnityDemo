...
#region Options
    ...
    [MenuItem("NGUI/Extras/Switch to Circle2D", false, 10)]
    static public void SwitchToCircle2D()
    {
        SphereCollider[] colliders = NGUITools.FindActive<SphereCollider>();

        for (int i = 0; i < colliders.Length; ++i)
        {
            SphereCollider c = colliders[i];
            GameObject go = c.gameObject;
            ...
            CircleCollider2D bc = go.AddComponent<CircleCollider2D> ();
            bc.radius = radius;
            bc.offset = center;
#if UNITY_4_3 || UNITY_4_5 || UNITY_4_6
			bc.center = center;

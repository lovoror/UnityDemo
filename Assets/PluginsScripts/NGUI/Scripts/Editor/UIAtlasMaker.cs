...
public class UIAtlasMaker : EditorWindow
{
	...
	{
		//dest texture will be imported multiple times, processor callback should be disabled here
		bool backup = WLAssetProcessor.shutup;
		WLAssetProcessor.shutup = true;
		...
		WLAssetProcessor.shutup = backup;
		ReimportAtlasTexture(atlas);
	}
	...
	void UpdateAtlas (List<Texture> textures, bool keepSprites)
	{
		...
		WLAssetProcessor.shutup = backup;
		ReimportAtlasTexture(NGUISettings.atlas);
	}
	...
	static void ReimportAtlasTexture(UIAtlas atlas)
	{

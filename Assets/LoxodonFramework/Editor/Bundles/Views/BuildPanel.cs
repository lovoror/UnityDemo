    ...
    public class BuildPanel : Panel
    {
        ...
        public override void OnEnable()
        {
            ...
            buildOptionList.Add(new CheckBoxData<BuildAssetBundleOptions>(
                ...
                (buildVM.BuildAssetBundleOptions & BuildAssetBundleOptions.AppendHashToAssetBundleName) > 0,
                BuildAssetBundleOptions.AppendHashToAssetBundleName));
            buildOptionList.Add(new CheckBoxData<BuildAssetBundleOptions>(
                "Exclude Type Information",
                "Do not include type information within the asset bundle (don't write type tree).",
                (buildVM.BuildAssetBundleOptions & BuildAssetBundleOptions.DisableWriteTypeTree) > 0,
                BuildAssetBundleOptions.DisableWriteTypeTree));
            buildOptionList.Add(new CheckBoxData<BuildAssetBundleOptions>(
                "Ignore Type Tree Changes",
                "Ignore the type tree changes when doing the incremental build check.",
               (buildVM.BuildAssetBundleOptions & BuildAssetBundleOptions.IgnoreTypeTreeChanges) > 0,
                BuildAssetBundleOptions.IgnoreTypeTreeChanges));
#if UNITY_5_6_OR_NEWER
            buildOptionList.Add(new CheckBoxData<BuildAssetBundleOptions>(
                "Strict Mode",
                "Do not allow the build to succeed if any errors are reporting during it.",
                (buildVM.BuildAssetBundleOptions & BuildAssetBundleOptions.StrictMode) > 0,
                BuildAssetBundleOptions.StrictMode));
            buildOptionList.Add(new CheckBoxData<BuildAssetBundleOptions>(
                "Dry Run Build",
                "Do a dry run build.",
                (buildVM.BuildAssetBundleOptions & BuildAssetBundleOptions.DryRunBuild) > 0,
                BuildAssetBundleOptions.DryRunBuild));
#endif

            outputPathContent = new GUIContent("Output Path", "");
            dataVersionContent = new GUIContent("Data Version", "The AssetBundle's data version.");
            usePlaySettingVersionContent = new GUIContent("Use PlayerSetting Version", "Use the PlayerSetting version.");
            copyToStreamingContent = new GUIContent("Copy to StreamingAssets", "After build completes, will copy all build content to Assets/StreamingAssets/bundles for use in stand-alone player.");
            useHashFilenameContent = new GUIContent("Use Hash Filename", "");
            encryptionContent = new GUIContent("Encryption", "Encrypts the AssetBundle's data.");
            algorithmContent = new GUIContent("Algorithm", "Choose AES128_CBC_PKCS7, AES192_CBC_PKCS7 or AES256_CBC_PKCS7");
            filterTypeContent = new GUIContent("Bundles", "");

            buildTargetContent = new GUIContent("Build Target", "Choose target platform to build for.");
        ...
        public override void OnGUI(Rect rect)
        {
            ...
            buildVM.BuildTarget = EditorUserBuildSettings.activeBuildTarget;
            EditorGUILayout.LabelField(buildTargetContent, new GUIContent(buildVM.BuildTarget.ToString()), GUILayout.Width(300f), GUILayout.Height(20));

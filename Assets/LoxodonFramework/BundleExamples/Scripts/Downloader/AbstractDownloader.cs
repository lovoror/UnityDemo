    ...
    public abstract class AbstractDownloader : IDownloader
    {
        ...
        protected virtual bool IsAllowedAbsoluteUri(Uri uri)
        {
            ...
            if ("http".Equals(uri.Scheme) || "https".Equals(uri.Scheme) || "ftp".Equals(uri.Scheme))
                return true;
#if UNITY_ANDROID
            if (uri.Scheme.Equals("jar", StringComparison.OrdinalIgnoreCase))
                return true;
#endif
        ...
        {
            string path = BaseUri.AbsoluteUri;
            if (BaseUri.Scheme.Equals("jar") && !path.StartsWith("jar:file://"))
